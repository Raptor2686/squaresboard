...
export default function MySquares() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/users/me/squares`, { credentials: "include" })
      .then((r) => r.json())
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="grid gap-4">
        {squares.map((sq) => (
          <div key={sq.id} className="bg-zinc-800 border border-zinc-700 rounded-xl p-4 flex items-center justify-between">
            <div>
              <p className="font-semibold">
                {sq.home_team} vs {sq.away_team}
              </p>
              <p className="text-sm text-zinc-400">
                {sq.quarter} · Board #{sq.game_id.slice(0, 8)} · Position {sq.position}
              </p>
            </div>
            <div className="text-right">
              ...
              <p className="text-sm text-zinc-400">{formatCents(sq.price_tier * 100)}</p>
            </div>
          </div>
        ))}
      </div>
